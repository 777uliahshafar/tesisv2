...
mrpa <- data.frame(
  Frek = colSums(df[7:10]),
  Pct.Resp = (colSums(df[7:10]) / sum(df[7:10])) * 100,
  Pct.Kasus = (colSums(df[7:10]) / nrow(df[7:10])) * 100
)
...
pendidikan <- subset(df$pendidikan, aksesibilitas >= 0 | keamanan >= 0 | estetika >= 0 | fasilitas >= 0)
...
ctpaD <- summarytools::ctable(pendidikan, df_aspekpro$aksesibilitas, dnn = c("Pendidikan", "Aksesibilitas"), prop= "t", round.digits = 0)
...
ctpkD <- summarytools::ctable(pendidikan, df_aspekpro$keamanan, dnn = c("Pendidikan", "Keamanan"), prop= "t", round.digits = 0)
...
ctpeD <- summarytools::ctable(pendidikan, df_aspekpro$estetika, dnn = c("Pendidikan", "Estetika"), prop="t", round.digits = 0)
...
ctpfD <- summarytools::ctable(pendidikan, df_aspekpro$fasilitas, dnn = c("Pendidikan", "Fasilitas"), prop="t", round.digits = 0)
...
# crosstabulation three var by didik
xtpaD <- xtabs(~pendidikan + df_aspekpro$aksesibilitas + ruang,data=df)

xtpkD <- xtabs(~pendidikan + df_aspekpro$keamanan + ruang,data=df)

xtpeD <- xtabs(~pendidikan + df_aspekpro$estetika + ruang,data=df)

xtpfD <- xtabs(~pendidikan + df_aspekpro$fasilitas + ruang,data=df)
...
\newcommand{\xtpabypen}{
\begin{table}\setlength\tabcolsep{2pt}
	\caption{Crosstabulasi aspek dan perbedaan pendidikan}
	\label{tab:xtpaD}
    \centering
\begin{tabular}[t]{p{1em}r P{4.3em}
    P{4.3em}P{4.3em}P{4.3em}P{4.3em}}
...
{\rotatebox[origin=c]{90}{\parbox[c]{2.2cm}{\textbf{Total}}}}\\
 \toprule
\multirow{3}{*}{Ra}&< SMA/ sederajat  & 4(3\%)    & 1(1\%)     & 6(4\%)  & 3(2\%) & 14(9\%)  \\
&SMA sederajat  & 18(12\%)  & 3(2\%)    & 14(9\%)   & 16(11\%) & 51(34\%)  \\
&Perguruan tinggi  & 10(7\%)  &1(1\%)    & 17(11\%)   & 11(7\%) & 39(26\%)  \\

\multirow{3}{*}{Rb}&< SMA/ sederajat  & 0  & 0     & 1  & 0 & 1(1\%)  \\
&SMA sederajat  & 12(8\%)  & 2(1\%)    & 3(2\%)   & 13(9\%) & 30(20\%)  \\
&Perguruan tinggi  & 3(2\%)  & 0    & 5(3\%)   & 7(5\%) & 15(10\%)  \\
&Total  & 47(31\%)  & 7(5\%)    & 46(31\%)   & 50(33\%) & 150(100\%)  \\
 \bottomrule

\multicolumn{7}{l}{\rule{0pt}{1em} \parbox{.8\textwidth}{Catatan: 1. Total respon adalah 150 \\ \hspace*{3.8em} 2. < sma/ sederajat berjumlah 9 orang ;  sma sederajat berjumlah 33 orang; perguruan tinggi berjumlah  43 orang}}\\
\multicolumn{7}{l}{\rule{0pt}{1em}\scriptsize Sumber: Analisis, 2022 }\\
\end{tabular}
\end{table}
}
...
<<mResEleFre, echo= F, error= F, message= F, results= "asis", warning= F, cache=F,fig.path="../figures/",fig.width=4, fig.height=4, autodep= T>>=

mrpe <- data.frame(
  Frek = colSums(df[11:18]),
  Pct.Resp = (colSums(df[11:18]) / sum(df[11:18])) * 100,
  Pct.Kasus = (colSums(df[11:18]) / nrow(df[11:18])) * 100
)

mrpe <- cbind(Elemen = rownames(mrpe), mrpe)
rownames(mrpe) <- NULL

mrpe <- mrpe %>% mutate(across(where(is.numeric), round, 2))

total <- c(size = "total", apply(mrpe[, -1], FUN = sum, MAR = 2))
mrpe_pro <- rbind(mrpe, total)

comm <- paste0("\\hline \n \\multicolumn{4}{l}",
               "{\\scriptsize{sumber: Analisis,2022 }} \n")

cat(paste("\\newcommand{\\mrpafre}{\n", sep=""))
print(xtable(mrpe_pro, align ="llldd" , hline = TRUE,
             row.labels = TRUE, percent = FALSE,
             caption = "Frekuensi Elemen", label="tab:mrpaF"),
      booktabs = TRUE, include.rownames = FALSE, caption.placement="top",add.to.row = list(pos = list(5), command = comm), sanitize.text.function = function(x) x,)
cat(paste("}\n"))
...
suku <- subset(df$usia, jmlhphn >= 0 | bntukphn >= 0 | lbrjln >= 0 | permjln >= 0| wrnabnga >= 0| jniskrs >= 0| tngkchy >= 0|ornele >= 0)

pendidikan <- subset(df$pendidikan, jmlhphn >= 0 | bntukphn >= 0 | lbrjln >= 0 | permjln >= 0| wrnabnga >= 0| jniskrs >= 0| tngkchy >= 0|ornele >= 0)
...
xtpjpD <- xtabs(~pendidikan + df_elemenpro$jmlhphn + ruang,data=df)
...
xtpbpD <- xtabs(~pendidikan + df_elemenpro$bntukphn + ruang,data=df)
...
xtpljD <- xtabs(~pendidikan + df_elemenpro$lbrjln + ruang,data=df)
...
xtppjD <- xtabs(~pendidikan + df_elemenpro$permjln + ruang,data=df)
...
xtpjkD <- xtabs(~pendidikan + df_elemenpro$jniskrs + ruang,data=df)
...
xtptcD <- xtabs(~pendidikan + df_elemenpro$tngkchy + ruang,data=df)
...
xtpoeD <- xtabs(~pendidikan + df_elemenpro$ornele + ruang,data=df)
...
\newcommand{\xtpebypen}{
\begin{table}\setlength\tabcolsep{0pt}
	\caption{Crosstabulasi elemen dan tingkat pendidikan}
	\label{tab:xtpeD}
    \centering
    \footnotesize
    \begin{tabular}[ht]{rP{1.5cm}P{1.5cm}P{1.5cm}P{1.4cm}P{1.5cm}P{1.4cm}P{1.4cm} }
\hline
\multirow{2}{*}{\bfseries\diagbox[innerleftsep=3pt,innerrightsep=3pt,width=8em, height=2.4cm]{Elemen \\ruang}{Kelompok}}&\multicolumn{3}{c}{\textbf{Ruang a}} &\multicolumn{3}{c}{\textbf{Ruang b}} & \textbf{Total}\\


&{\rotatebox[origin=cB]{35}{\parbox[b]{2.4cm}{< sma\ sederajat}}}&{\rotatebox[origin=B]{35}{\parbox[b]{2.4cm}{sma sederajat}}}&{\rotatebox[origin=B]{35}{\parbox[b]{2.4cm}{Peruguran tinggi}}}&{\rotatebox[origin=B]{35}{\parbox[b]{2.4cm}{< sma\ sederajat}}}&{\rotatebox[origin=B]{35}{\parbox[b]{2.4cm}{sma sederajat}}}&{\rotatebox[origin=B]{35}{\parbox[b]{2.4cm}{Perguruan tinggi}}} & \\

\toprule

Jumlah pohon  & 2 (1\%)   & 8 (3\%)    & 6 (2\%)  & 0 & 1 & 3 (1\%) &    20 (7\%)\\
Bentuk pohon  & 4 (1\%)    & 14 (5\%)   & 12 (4\%)  &1 & 1 (\%) & 8 (3\%) & 40(15\%) \\
Lebar jalan  & 6 (2\%)     & 20 (7\%)    & 12 (4\%)    & 1  & 4 (1\%) & 8 (3\%) & 51(19\%) \\

Permukaan jalan  & 4 (1\%)    & 15 (6\%)    & 12 (4\%)    & 0 (\%) & 2 (1\%) & 4(1\%) & 37(14\%)\\
Jenis kursi& 1   & 6 (2\%)  & 2 (1\%)    & 0   & 0  & 2(1\%) & 11(4\%)  \\
Tingkat cahaya  & 4 (1\%)  & 16 (6\%)   &16 (6\%)   & 1  & 8 (3\%) & 8 (3\%) & 53(20\%)\\
Orientasi elemen  & 6 (2\%)  & 14 (5\%)   &17 (6\%)   & 1  & 5 (2\%) & 12 (4\%) & 55(21\%)\\

\textbf{Total}  & 27 (10\%)  & 93 (35\%)   & 77 (29\%)   & 4 (1\%) & 21 (8\%) & 45(17\%) & 267(100\%)\\

 \bottomrule
\multicolumn{8}{l}{\rule{0pt}{1em} \parbox{.8\textwidth}{Catatan: 1. Total respon adalah 267 \\ \hspace*{3.8em} 2. < sma sederajat  berjumlah 9 orang ;  sma sederajat berjumlah 33 orang; perguruan tinggi berjumlah  43 orang}}\\
\multicolumn{8}{l}{\rule{0pt}{1em}\scriptsize Sumber: Analisis, 2022 }\\
\end{tabular}
\end{table}
}
