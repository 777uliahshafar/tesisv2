...
ggplot(
  ...
  scale_fill_manual(values = mycols) +
  theme_void()
cat(paste("\\caption{Pie Chart Gender}\n", sep=""))
cat("\\label{pcg}")
cat("\\end{figure}\n")
cat(paste("}\n"))
@
...
usia <- cut(df$usia, breaks = c(5, 11, 25, 45, 65), labels = c("anak-anak", "remaja", "dewasa", "lansia"), right = T)
df$usia <- usia
...
ggplot(
  ...
  scale_fill_manual(values = mycols) +
  theme_void()
cat(paste("\\caption{Pie Chart Usia}\n", sep=""))
cat("\\label{pcu}")
cat("\\end{figure}\n")
cat(paste("}\n"))
...
ggplot(
  ...
  scale_fill_manual(values = mycols) +
  theme_void()
cat(paste("\\caption{Pie Chart Suku}\n", sep=""))
cat("\\label{pcs}")
cat("\\end{figure}\n")
cat(paste("}\n"))
...
<<grpbarpek, echo= F, error= F, message= F, results= "asis", warning= F, cache=F,fig.path="../figures/",fig.width=4, fig.height=3, autodep= T>>=
...
<<cTabPek, echo= F, error= F, message= F, results= "asis", warning= F, cache=F,fig.path="../figures/",fig.width=4, fig.height=3, autodep= T>>=
cTabPek <- xtabs(~ pekerjaan + ruang, data = df)

#cTabPek <- table(df$pekerjaan,df$gender,df$ruang)


cat(paste("\\newcommand{\\cTabPek}{\n", sep=""))
print(xtable(cTabPek,
             label="Cross Tab Pekerjaan",
             caption=paste("Cross Tab 1"), digit=3 ))
cat(paste("}\n"))

@


<<cTabSuku, echo= F, error= F, message= F, results= "asis", warning= F, cache=F,fig.path="../figures/",fig.width=4, fig.height=3, autodep= T>>=

cTabSuku <- xtabs(~ suku + ruang, data = df)
chisSuku <- chisq.test(cTabSuku)

##Create table for chisquare to xtable
df.chisSuku <- data.frame(D = c(df$suku, df$ruang),
                     `p value` = c(chisSuku$p.value,chisSuku$p.value))

cat(paste("\\newcommand{\\cTabSuku}{\n", sep=""))
print(xtable(df.chisSuku,
             label="Cross Tab Gender",
             caption=paste("Cross Tab 1"), digit=3 ))
cat(paste("}\n"))

@

<<cTabSukuPlus, echo= F, error= F, message= F, results= "asis", warning= F, cache=F,fig.path="../figures/",fig.width=4, fig.height=3, autodep= T>>=

ct_suku <- crosstab(df$ruang, df$suku, dnn = c("Ruang", "Suku"), expected = TRUE, prop.c = TRUE, prop.r = TRUE, plot = FALSE)

#tab_suku <- ct_suku$tab
#class(tab_suku)
#tab_suku

tab_plus_suku <- descr:::CreateNewTab(ct_suku)
#class(tab_plus_suku)
#tab_plus_suku

cat(paste("\\newcommand{\\cTabSukuPlus}{\n", sep=""))
print(xtable(tab_plus_suku, align = "lddddd", multirow = TRUE, hline = TRUE, row.labels = TRUE, percent = FALSE,label="ctsukuplus",caption="Crosstab Suku plus"),booktabs = TRUE, include.rownames = FALSE)
cat(paste("}\n"))
